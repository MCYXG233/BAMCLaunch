...
void main() {
  group('GameOutputMonitor.parseLogLevel', () {
    ...
    test('包含 error 应识别为 error', () {
      expect(
        monitor.parseLogLevel('java.lang.Error occurred'),
        equals(GameLogLevel.error),
      );
      expect(
        monitor.parseLogLevel('Exception in thread'),
        equals(GameLogLevel.error),
      );
    });
    ...
    test('FATAL 关键字不在 parseLogLevel 识别范围内（返回 info）', () {
      ...
      expect(
        monitor.parseLogLevel('FATAL: something broke'),
    ...
    test('包含 warn 应识别为 warn', () {
      expect(
        monitor.parseLogLevel('WARNING: deprecated API'),
        equals(GameLogLevel.warn),
      );
      expect(
        monitor.parseLogLevel('WARN - low memory'),
        equals(GameLogLevel.warn),
      );
    });
    ...
    test('包含 debug 应识别为 debug', () {
      expect(
        monitor.parseLogLevel('DEBUG: initializing'),
    ...
    test('普通输出应识别为 info', () {
      expect(
        monitor.parseLogLevel('Loading Minecraft 1.20.4'),
        equals(GameLogLevel.info),
      );
    ...
    test('大小写不敏感', () {
      expect(monitor.parseLogLevel('ERROR: oops'), equals(GameLogLevel.error));
      expect(monitor.parseLogLevel('Warning: x'), equals(GameLogLevel.warn));
    });
  });
  ...
  group('GameOutputMonitor - 集成测试范围', () {
    test('startMonitoring 是公开方法（接口验证）', () {
      ...
      // 通过反射方法列表验证 API 存在
      // 避免在单测中实际启动 Process
      expect(monitor.runtimeType.toString(), equals('GameOutputMonitor'));
    });
  });
}
