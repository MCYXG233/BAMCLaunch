...
class XboxAuthService {
  ...
  Future<XboxLiveToken> authenticateUser(String accessToken) async {
    ...
    final response = await networkClient.postJson(_userAuthenticateEndpoint, {
      'Properties': {
        'AuthMethod': 'RPS',
    ...
    try {
      ...
      throw AppException.fromCode(
        ...
        detail:
            'Xbox auth returned invalid JSON: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}',
        originalError: e,
      );
    }
    final Map<String, dynamic> displayClaims =
        data['DisplayClaims']['xui'][0] as Map<String, dynamic>;
  ...
  Future<XstsToken> acquireXstsToken(String xboxLiveToken) async {
    final networkClient = NetworkClient();
    final response = await networkClient.postJson(_xstsAuthenticateEndpoint, {
      'Properties': {
        'SandboxId': 'RETAIL',
    ...
    if (response.statusCode != 200) {
      ...
      try {
        ...
        throw AppException.fromCode(
          ErrorCodes.networkJsonParseError,
          detail:
              'XSTS error returned invalid JSON: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}',
          originalError: e,
        );
      }
      ...
      if (errorCode == '2148916227') {
        throw AppException.fromCode(
          ErrorCodes.authXboxFailed,
          detail: '账户已被Xbox Live封禁 (2148916227)',
        );
      } else if (errorCode == '2148916233') {
        throw AppException.fromCode(
          ErrorCodes.authXboxFailed,
          detail: '账户没有Xbox Live (2148916233)',
        );
      } else if (errorCode == '2148916235') {
        throw AppException.fromCode(
          ErrorCodes.authXboxFailed,
          detail: '当前地区不可用Xbox Live (2148916235)',
        );
      } else if (errorCode == '2148916238') {
        throw AppException.fromCode(
          ErrorCodes.authXboxFailed,
          detail: '子账户需要家庭设置 (2148916238)',
        );
      }
    ...
    try {
      ...
      throw AppException.fromCode(
        ErrorCodes.networkJsonParseError,
        detail:
            'XSTS success returned invalid JSON: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}',
        originalError: e,
      );
    }
    final Map<String, dynamic> displayClaims =
